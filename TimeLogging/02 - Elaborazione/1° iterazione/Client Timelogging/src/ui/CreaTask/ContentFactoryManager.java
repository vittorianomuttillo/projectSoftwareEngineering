...
public class ContentFactoryManager implements ContentFactory, TreeSelectionListener, ActionListener {
    ...
    private static String CLEAR_COMMAND = "Elimina Task";
	...
	public JPanel createTreePanel() {
		...
		ContentView = new JScrollPane(InfoPane);
	...
	private void displayInfoPane() {    
		InfoPane.setText("Informazioni generali del manager");	            
	}
	...
	public void valueChanged(TreeSelectionEvent arg0) {
	    ...
	    if (node.isRoot()){
	    	...
	    	GestisciTask.setSottoprogetto(node.getParent().toString());
	    	GestisciTask.setProgetto(this.getGroupProgetti().get(node.getParent().toString()));
	    	...
	    	this.updateButton.setEnabled(false);
	    	this.clearButton.setEnabled(false);
	    	GestisciTask.setSottoprogetto((String)node.getUserObject());
	    	GestisciTask.setProgetto(this.getGroupProgetti().get((String)node.getUserObject()));
	    } 
	}
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (ADD_COMMAND.equals(command)) {
			...
			GestisciTask  gestisci = new GestisciTask(f, "Modifica");
			gestisci.getNomeTask().setText(task[0]);
			gestisci.getDescrizioneTask().setText(task[1]);
			gestisci.getConsulente().setEnabled(false);
			gestisci.getTariffa().setEnabled(false);
			gestisci.getUnita().setEnabled(false);
			gestisci.getQuantita().setEnabled(false);
			GestisciTask.setOld(task[0]);
