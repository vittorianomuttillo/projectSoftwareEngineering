...
public class ETaskDAO {
	...
	public static boolean save(domain.entity.ETask eTask) {
		try {
			persistence.DAO.TimeloggingPersistentManager.instance().saveObject(eTask);
			return true;
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public static boolean delete(domain.entity.ETask eTask) {
		try {
			persistence.DAO.TimeloggingPersistentManager.instance().deleteObject(eTask);
			return true;
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public static boolean deleteAndDissociate(domain.entity.ETask eTask) {
		try {
			domain.entity.ETariffa[] leTariffass = (domain.entity.ETariffa[])eTask.geteTariffas().toArray(new domain.entity.ETariffa[eTask.geteTariffas().size()]);
			for(int i = 0; i < leTariffass.length; i++) {
				leTariffass[i].seteTask(null);
			}
			return delete(eTask);
		}
		catch(Exception e) {
	...
	public static boolean deleteAndDissociate(domain.entity.ETask eTask, org.orm.PersistentSession session) {
		try {
			domain.entity.ETariffa[] leTariffass = (domain.entity.ETariffa[])eTask.geteTariffas().toArray(new domain.entity.ETariffa[eTask.geteTariffas().size()]);
			for(int i = 0; i < leTariffass.length; i++) {
				leTariffass[i].seteTask(null);
	...
	public static boolean refresh(domain.entity.ETask eTask) {
		try {
			persistence.DAO.TimeloggingPersistentManager.instance().getSession().refresh(eTask);
			return true;
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public static boolean evict(domain.entity.ETask eTask) {
		try {
			persistence.DAO.TimeloggingPersistentManager.instance().getSession().evict(eTask);
			return true;
		}
		catch (Exception e) {
			e.printStackTrace();
